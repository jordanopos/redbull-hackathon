...
export default function ActionsPage() {
  ...
  const [linkedInEditMode, setLinkedInEditMode] = useState<"manual" | "ai">("manual");
...
The Hackathon Team`
  ...
  const storyHighlight = "Our third annual hackathon brought together 156 innovators, resulting in 23 groundbreaking projects and fostering unprecedented collaboration between students and industry mentors.";
...
#Hackathon #Innovation #TechForGood #CodingCommunity`;
  ...
  const handleEditEmail = () => {
    const emailContent = emails.find(e => e.id === selectedEmail)?.content || "";
  ...
  const handleEmailAIPromptChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handleLinkedInAIPromptChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
  ...
  const handleGenerateLinkedInWithAI = () => {
    // In a real app, this would call an AI service
    // For now, we'll just modify the content as a simulation
    setLinkedInPostContent(`${linkedInPostContent}\n\n[Enhanced with AI based on: "${linkedInAIPrompt}"]`);
    // Reset prompt after "generating"
    setLinkedInAIPrompt("");
  };
            ...
            <div className="col-span-1 border-r pr-4">
              ...
              <ul className="space-y-2">
                {emails.map(email => (
                  <li key={email.id}>
                    <button 
                      className={`w-full text-left p-2 rounded hover:bg-yellow-50 ${selectedEmail === email.id ? 'bg-yellow-50 border border-yellow-200' : ''}`}
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex space-x-2">
                        ...
                        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                          ...
                          <div className="mb-4">
                            <div className="flex border-b mb-4">
                              <button 
                                className={`py-2 px-4 ${emailEditMode === 'manual' ? 'border-b-2 border-[#ffbd00] text-[#ffbd00]' : 'text-gray-500'}`}
                                onClick={() => handleEditModeChange('manual')}
                              >
                                Manual Edit
                              </button>
                              <button 
                                className={`py-2 px-4 ${emailEditMode === 'ai' ? 'border-b-2 border-[#ffbd00] text-[#ffbd00]' : 'text-gray-500'}`}
                              ...
                              <div>
                                ...
                                <div className="mb-4">
                                  <label className="block text-sm text-gray-600 mb-2">
                                    Describe how you&apos;d like to improve this email:
                                  </label>
                                  <textarea 
                      ...
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button 
                            className="bg-[#ffbd00] hover:bg-yellow-600 whitespace-nowrap"
                          >
                            Send Email
                          </Button>
                        </DialogTrigger>
      ...
      <div className="mb-10">
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="mb-4 flex justify-between items-center">
            ...
            <Button 
              className={`bg-[#ffbd00] hover:bg-yellow-600 ${selectedQuestions.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <ul className="space-y-2">
            {surveyQuestions.map((question, index) => (
              <li key={index} className="flex items-center">
                ...
                />
                <label htmlFor={`question-${index}`} className="select-none">{question}</label>
              </li>
            ))}
          </ul>
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="mb-4">
                          <div className="flex border-b mb-4">
                            <button 
                              className={`py-2 px-4 ${linkedInEditMode === 'manual' ? 'border-b-2 border-[#ffbd00] text-[#ffbd00]' : 'text-gray-500'}`}
                              onClick={() => handleLinkedInEditModeChange('manual')}
                            >
                              Manual Edit
                            </button>
                            <button 
                              className={`py-2 px-4 ${linkedInEditMode === 'ai' ? 'border-b-2 border-[#ffbd00] text-[#ffbd00]' : 'text-gray-500'}`}
                          ...
                          {linkedInEditMode === 'manual' ? (
                            ...
                            <div>
                              ...
                              <div className="mb-4">
                                <label className="block text-sm text-gray-600 mb-2">
                                  Describe how you&apos;d like to improve this post:
                                </label>
                                <textarea 
                                  className="w-full p-3 border rounded h-24"
